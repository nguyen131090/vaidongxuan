...
<?php $this->registerCssFile('/assets/css/mobile/associations.css',['depends' => 'app\assets\AppAsset', 'position' => $this::POS_END]) ?>
<div class="contain container-2 non-area-form">
    ...
    class="banner-img lazyload" />
                <? }else{ ?>
             <img class="image-banner lazyload" alt="" data-src="<?=DIR?>assets/img/mobile/img-destination-ideel-banner-640-442.jpg">
                <? }?>
...
<span class="space space-50"></span>
        <div class="ajaxfilter">
                                ...
                                <a href="<?=DIR.$v->slug?>">
                                     <? if(isset($v->photosArray['summary'])){
                                        $value = $v->photosArray['summary'][0];
                                                    echo '<img data-sizes="auto" alt="'.$value->description.'" class="lazyload" data-srcset="'.$value->image.'">';
                                        }
                                    ?>
                                    <span class="space space-50"></span>
                                    <h2 class="no-margin tt-latolatin-bold tt-fontsize-40">
                                        <?= $v->title?>
                                    </h2>
            ...
            <?
             ...
             if(count($totalCount) && (count($totalCount) / 12) > $page ){?>  
...
<div class="contain container-6 contain-background-ededed">
    <div class="column">
        <span class="space space-50"></span>
        <p class="tt-custom">Notre équipe à votre écoute</p>
        <span class="space space-50"></span>
        <span class="btn-link btn-link-1 tt-title tt-fontsize-32 tt-latolatin-bold tt-color-e65925 pugjd" data-title="<?= base64_encode(DIR.'devis') ?>"><span>Faites-nous savoir vos attentes</span></span>
