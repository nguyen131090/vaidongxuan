...
<!--        <span class="space fix-space-9-3rem"></span>-->
        <div data-role="header" class="" style="min-height: 5rem;">
			...
			<a href="<?=DIR?>#page1" data-transition="slidedown" data-direction="" class='btn-close tt-title tt-latolatin-bold tt-fontsize-32 tt-color-white'>
...
<!--				<img src="/assets/img/mobile/close-icon.png">-->
                ...
                <div class="area-reset-filter">
                    <span class="reset-filter tt-title tt-latolatin-bold tt-fontsize-32 tt-color-white" data-get="country=all&type=all&length=all&region=all">Effacer vos filtres</span>
                </div>
            </div>        
	</div><!-- /header -->
        <div class="updatefilter" data-url="<?=$this->context->arr_option_filter_voyage_mobile['uri']?>">
            ...
            ?>    
            <p class="tt-title tt-latolatin-regular tt-fontsize-40 tt-color-black tt-custom <?= $classhide ?>" style="<?= $hide == 1 ? 'display: none;' : '' ?>">Destination(s)</p>
            <ul class="options destination <?= $classhide ?>">
                <? foreach (Yii::$app->params['tsDestinationListNew'] as $key => $value) { ?>
                    <li data-option="" class="<?= in_array($key, $country) ? 'selected ' : ($this->context->arr_option_filter_voyage_mobile['country'] == SEG1 ? $switch_link : '') ?>" data-value="<?= $key ?>" style="<?= $this->context->arr_option_filter_voyage_mobile['numberFilter']['countCountry'][$key] == 0 ? 'display: none;' : '' ?>" data-analytics="on" data-analytics-category="overlay" data-analytics-action="sugg_section" data-analytics-label="select_dest_opt_<?= $key ?>"><?= $value?><span class="nb">&nbsp;(<?= $this->context->arr_option_filter_voyage_mobile['numberFilter']['countCountry'][$key] ?>)</span></li>                   
                <? } ?>
            ...
            ?>
            <p class="tt-title tt-latolatin-regular tt-fontsize-40 tt-color-black tt-custom" style="<?= $hide == 1 ? 'display: none;' : '' ?>">Durée</p>
            <ul class="options tour-length">
                <? foreach (Yii::$app->params['tsDurationListNewMobile'] as $key => $value) : ?>
                <li data-option="" class="<?= in_array(explode('-', $key)[0], $length) ? 'selected' : '' ?>" data-value="<?= $key ?>" style="<?= $this->context->arr_option_filter_voyage_mobile['numberFilter']['countLength'][$key] == 0 ? 'display: none;' : '' ?>" data-analytics="on" data-analytics-category="overlay" data-analytics-action="sugg_section" data-analytics-label="select_duration_opt_<?= $key ?>"><?= $value ?><span class="nb">&nbsp;(<?= $this->context->arr_option_filter_voyage_mobile['numberFilter']['countLength'][$key] ?>)</span></li>
                <? endforeach; ?>
            ...
            ?>
            <p class="tt-title tt-latolatin-regular tt-fontsize-40 tt-color-black tt-custom" style="<?= $hide == 1 ? 'display: none;' : '' ?>">Type de voyage</p>
            <ul class="options  voyage">
                <? foreach (\app\modules\programmes\models\Category::find()->roots()->all() as $key => $value) : ?>
                    <li data-option="" class="<?= in_array($value->category_id, $type) ? 'selected ' : ($this->context->arr_option_filter_voyage_mobile['country'] == SEG1 ? '' : $switch_link) ?>" data-value="<?=$value->category_id ?>" style="<?= $this->context->arr_option_filter_voyage_mobile['numberFilter']['countType'][$value->category_id] == 0 ? 'display: none;' : '' ?>" data-analytics="on" data-analytics-category="overlay" data-analytics-action="sugg_section" data-analytics-label="select_voyage_opt_<?= $value->category_id ?>"><?=$value->title ?><span class="nb">&nbsp;(<?= $this->context->arr_option_filter_voyage_mobile['numberFilter']['countType'][$value->category_id] ?>)</span></li>
                <? endforeach; ?>
            </ul>
            <div class="clearfix"></div> 
            <? 
            ...
            <ul class="options tour-region">
                <? foreach (Yii::$app->params['tRegionList'] as $key => $value) : ?>
                    <li data-option="" class="<?= in_array(explode('-', $key)[0], $region) ? 'selected' : '' ?>" data-value="<?= $key ?>" style="<?= $this->context->arr_option_filter_voyage_mobile['numberFilter']['countRegion'][$key] == 0 ? 'display: none;' : '' ?>" data-analytics="on" data-analytics-category="overlay" data-analytics-action="sugg_section" data-analytics-label="select_region_opt_<?= $key ?>"><?= $value ?><span class="nb">&nbsp;(<?= $this->context->arr_option_filter_voyage_mobile['numberFilter']['countRegion'][$key] ?>)</span></li>
                <? endforeach; ?>
               
                </ul> 
            
            <div class="space space-160"></div>
            ...
            <div data-role="footer"  data-position="fixed" data-tap-toggle="false" class="fix-button-filter ui-footer ui-bar-inherit ui-footer-fixed slideup">
                <span class="cs-select submit <?= $this->context->arr_option_filter_voyage_mobile['totalCount'] == 0 ? 'disable' : '' ?>" data-analytics="on" data-analytics-category="overlay" data-analytics-action="sugg_section" data-analytics-label="btn_voyage">
