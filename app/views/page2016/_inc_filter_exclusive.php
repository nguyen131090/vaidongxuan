...
?>
<?php 
    if(($request_country == 'all' || $request_country == '' || $request_country == null) && ($request_type == 'all' || $request_type == '' || $request_type == null) && ($request_region == 'all' || $request_region == '' || $request_region == null)){
        $classActive = null;
    }else{
        $classActive = 'active';
    }
...
<?php if((Yii::$app->request->get('country') == 'all' && Yii::$app->request->get('type') == 'all' && Yii::$app->request->get('region') == 'all') || (Yii::$app->request->get('country') == '' && Yii::$app->request->get('type') == '' && Yii::$app->request->get('region') == '')){?>
...
<div class="area-votre-selection">
    <div class="votre-selection <?= $classActive ?>">
        ...
        <p class="remove-result-filter mt-20 mb-20 <?= $classActive ?>" data-get="country=all&type=<?= $typeId ?>&region=all">Effacer vos filtres</p>
...
<div class="filter-number">
    <div class="search-form getnumber">
        <form class="form-search vertical search-excl-form">
           ...
           <div class="cs-select search-type search-envies votre-envie-mn-2 filter-type <?= $first_act ?>">
                ...
                <div class="cs-options">
                    <ul>
                        ...
                        <? foreach ($type = \app\modules\exclusives\models\Category::find()->roots()->orderBy('order_num desc')->all() as $key => $value) : ?>
                            <li class="<?= $switch_link ?> <?= in_array($value->category_id, $selectType) ? 'active' : '' ?>" data-option="" data-value="<?= $value->category_id ?>" style="<?= $numberFilter['countType'][$value->category_id] == 0 ? 'display: none;' : ''?>" data-analytics="on" data-analytics-category="formules_page" data-analytics-action="filter_section" data-analytics-label="opt_inspi_secret_<?= $value->category_id  ?>"><?= $value->title ?><span class="nb"> (<?= $numberFilter['countType'][$value->category_id] ?>)</span></li>
                        <? endforeach ?>
                    </ul>
                </div>
                <div class="list-option">
