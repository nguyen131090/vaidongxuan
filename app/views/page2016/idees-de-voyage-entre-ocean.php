                ...
                <div id='load-more' class="getcontent">
                    ...
                    foreach ($theEntries as $v) {
                        ...
                        <div id="<?=$cnt?>" class="item item-<?= $cnt ?> <?= $cnt % 2 != 0 ? 'it-l ml-0' : 'it-r mr-0' ?>">
                                ...
                                if(!empty($v->photos)){
                                    ...
                                    foreach ($v->photos as $value) {
                                        ...
                                        if($type == 'summary'){
                                            ...
                                            ?>
                                            <img style="width: 100%" class="img-lazy" alt="<?= $value->description?>" <?= Yii::$app->request->isAjax ? 'src="'.$value->image.'"' : 'data-src="'.$value->image.'"' ?>>
                                            <?php
                                        }
                                    }
                            ...
                            ?>
                            <ul class="<?php echo $hidden ?>">
                                <?php if(isset($v->model)){?>
                                    <li class="posi">
                                        <p class="posi-title">
                    ...
                    if(Yii::$app->request->get('country') != NULL && Yii::$app->request->get('length') != NULL && Yii::$app->request->get('type') != NULL){
                        if($totalCount > $seemore){?>
                            <div class="see-more">
                                <span class="btn-submit ajax-see-more btn-amica-basic-1 btn-amica-basic" data-get="<?= 'country='.$country.'&type='.$type.'&length='.$length.'&see-more='.(12 + $seemore) ?>" data-value='<?= count($theEntries) ?>'>Plus de circuits <span>arrow down</span></span>
                            </div>
                        <? } }?>
