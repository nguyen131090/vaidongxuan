                        ...
                        <? $i = ''; ?>
                                    <? foreach ($value->photos as $kp => $vp) : ?>
                                        <? if($vp->type == 'summary')
                                                    $i = $kp; ?>
                                    <? endforeach; ?>
                            <? if($i) : ?> <img width="460" height="<?=$key == 4 ? '587' : '281'?>" alt="<?=$value->photos[$i]->description?>" data-src="<?=$value->photos[$i]->image?>" class="img-lazy lazyload"/>
                        <? endif; ?>
...
<!--                        <div class="text-slide">
                ...
                <? if($key == 3) echo "</li><li class='slider-2'>"; ?>
...
<div class="entry-body-entry container-flud mb-60 responsive-entry-body">     
    ...
    <div class="column pourquoi <?= SEG1?> mt-0">
        <h2 class="title-pourquoi mt-txt-0 mb-txt-40">Pourquoi nous choisir pour votre voyage au <?= ucwords(SEG1) ?></h2>   
        <p class="text">Depuis 2007, nous organisons et créons des voyages sur mesure au Cambodge. Notre cœur de métier : Création des circuits hors de sentiers battus, authentiques, axés sur la découverte des paysages exceptionnels et caractéristiques, sur l’immersion à l’intérieur de la vie locale, sur les rencontres avec les locaux.</p> 
    ...
    <div class="column preparer-voyage mt-60">
        ...
        <a href="https://www.amica-travel.com/<?=SEG1?>/informations-pratiques" style="color: #e35429;" class="btn-amica-basic btn-amica-basic-1 mt-txt-40 mb-0" data-analytics="on" data-analytics-category="country_page" data-analytics-action="info_section" data-analytics-label="cta_c_infos">Voir toutes nos informations pratiques sur le <?= ucfirst(SEG1) ?></a>
        <div class="btn-preparer mt-60">
            <div>
                ...
                <?= \yii\helpers\Html::dropDownList('search_destination', null, $optDes,[
                        'class' => 'chosen',
                        'multiple' => 'multiple', 
                        'data-placeholder' => 'Tapez le nom d’un site (Sieam Reap,...)',
...
$css = <<<CSS
    .bx-wrapper .text-slide{
        background-color: rgba(202,139,9,0.7);
    }
