                    ...
                    <ul class="bxslider" id="bxslider-programes" >
                        ...
                        <li>
                            <div data-name="programes" class="item item-<?=$kps+1 ?>">
                                <a class="url-tour" href="<?=DIR.$vps->slug?>">
                                    <? foreach ($vps->photos as $kpvps => $pvps) :?>
                                        <? if($pvps->model->type == 'summary') : ?>
                                                <img width="194" height="129"  src="<?='/timthumb.php?src='.$pvps->image.'&w=194&zc=0'?>" alt="<?=$pvps->description?>">
                                        <? endif; ?>
                                    <? endforeach; ?>
                                ...
                                <a href="javascript:void(0)" data-id="<?= $vps->model->item_id?>" data-type="prog" class="btn-add" name="selected"></a>
                                <? else: ?>
                        ...
                        <li>
                            <div data-name="exclusives" class="item item-<?=$kes+1 ?>">
                                <a class="url-tour" href="<?=DIR.$ves->slug?>">
                                    <? foreach ($ves->photos as $kesp => $vesp) :?>
                                        <? if($vesp->model->type == 'summary') : ?>
                                                <img   width="194" height="129"    src="<?='/timthumb.php?src='.$vesp->image.'&w=194&zc=0'?>" alt="<?=$vesp->description?>">
                                        <? endif; ?>
                                    <? endforeach; ?>
                               ...
                               <a href="<?=DIR.$ves->slug.'?tab=form'?>" class="link-to-form">Personnaliser cette formule</a>    
                                ...
                                <a href="javascript:void(0);" data-id="<?= $ves->model->item_id?>" data-type="excl" class="btn-add" name="selected"></a>
                                <? else: ?>
