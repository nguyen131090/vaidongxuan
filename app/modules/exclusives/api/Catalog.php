...
use app\modules\exclusives\models\ItemData;
...
/**
 * Catalog module API
 * @package app\modules\exclusives\api
 *
...
class Catalog extends \yii\easyii\components\API
{
    ...
    'app_exclusives_items.*', 
    new \yii\db\Expression('IF(on_top IS NULL OR on_top="0", 1, 0) AS on_top_flag')
])->joinWith('category')->with(['seo','category'])->andWhere(['app_exclusives_items.status' => 1]);

            if(!empty($options['where'])){
                $query->andFilterHaving($options['where']);
            }
            if(!empty($options['orderBy'])){
              //  $query->orderBy(['order_num' => SORT_ASC]);
