...
class CategoryObject extends \yii\easyii\components\CategoryApiObject
{
    ...
    public function parents(){
        if(!$this->_parents){
            $this->_parents = \app\modules\destinations\models\Category::findOne(['category_id' => $this->id])->parents()->all();
        }
    ...
    public function children(){
        if(!$this->_children){
            $this->_children = \app\modules\destinations\models\Category::findOne(['category_id' => $this->id])->children()->select([
    '*', 
    new \yii\db\Expression('IF(on_top IS NULL OR on_top="0", 100, on_top) AS on_top_flag')
])->andWhere(['status' => 1])->orderBy('order_num ASC, lft ASC')->all();
        }
