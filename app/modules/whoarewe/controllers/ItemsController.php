...
ORDER BY 'id', 'group'")->queryAll();
    ...
    public function getModulesPage(){
        $programes = $exclusives = $destinations = $temoignages = $blogs = [];
...
ORDER BY 'id', 'group'")->queryAll();
    ...
    public function getDestinationData(){
        $parents = ['vietnam/visiter', 'cambodge/visiter', 'laos/visiter', 'birmanie/visiter'];
        foreach($parents as $kp => $vp){
            $child = \app\modules\destinations\models\Category::findOne(['slug' => $vp])->children()->all();
            foreach ($child as $kc => $vc) {
                $parents[] = $vc->slug;
            }
        }
        
        $dataDes = \app\modules\destinations\models\Category::find()->where(['slug' => $parents])->with('items')->asArray()->all();
            $destinations = [];
            foreach ($dataDes as $key => $value) {
                $destinations[$value['sub_title']] = ArrayHelper::map($value['items'], 'slug', 'title');
            }
        return $destinations;
    }
...

    {
...

        {
            ...
            elseif ($field->type === 'selectLocations') {
                $result .= '<div class="control-group">
                    <label for="select-beast">'.$field->title.'</label>
                   <select id="list-exts-'.$field->name.'" data-type = "'.$field->name.'" name="'."Data[{$field->name}][]".'" multiple class="form-control selectTags">'. Html::renderSelectOptions($value, $this->getDestinationData()) .'</select>
                </div>';
            ...
            elseif ($field->type === 'exclusives') {
                $result .= '<div class="control-group">
                    <label for="select-beast">'.$field->title.'</label>
                   <select id="list-exts-'.$field->name.'" data-type = "'.$field->name.'" name="'."Data[{$field->name}][]".'" multiple class="form-control selectTags">'. Html::renderSelectOptions($value, $this->getExclu()) .'</select>
                </div>';
                $result .= '<input style="display: none;" id="exts-order-'.$field->name.'" name="exts-order['.$field->name.']" />';
            ...
            elseif ($field->type === 'extension') {
                ...
                $result .= '<div class="control-group">
                    <label for="select-beast">'.$field->title.'</label>
                   <select name="'."Data[{$field->name}][]".'" multiple class="form-control selectTags">'. Html::renderSelectOptions($value, $this->getModulesPage()['programes']) .'</select>
                </div>';
            ...
            elseif ($field->type === 'libraries') {
                $result .= '<div class="control-group">
                    <label for="select-beast">'.$field->title.'</label>
                   <select id="list-exts-'.$field->name.'" data-type = "'.$field->name.'" name="'."Data[{$field->name}][]".'" multiple class="form-control selectTags">'. Html::renderSelectOptions($value, $this->getLibrariesAll()) .'</select>
                </div>';
                $result .= '<input style="display: none;" id="exts-order-'.$field->name.'" name="exts-order['.$field->name.']" />';
...
ORDER BY 'id', 'group'")->queryAll();
     ...
     public function getLibrariesAll(){

         $libs = Yii::$app->db->createCommand("SELECT item.item_id as 'id', item.slug as 'slug', item.title as 'text', cat.title as 'group' FROM app_libraries_items 
 as item JOIN app_libraries_categories as cat ON cat.category_id = item.category_id
WHERE item.category_id
ORDER BY 'id', 'group'")->queryAll();
            foreach ($libs as $kl => $vl) {
                $lib[$vl['group']][$vl['id']] = $vl['text'];
            }
        return $lib;
    }
