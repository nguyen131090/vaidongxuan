...
class ItemObject extends \yii\easyii\components\ItemApiObject
{
    ...
    public function parents($number = null){
        if($number){
            $parents = \app\modules\whoarewe\models\Category::findOne(['category_id' => $this->category_id])->parents($number)->one();
        } else{
            $parents = \app\modules\whoarewe\models\Category::findOne(['category_id' => $this->category_id])->parents()->all();
            $parents[] = $this->getCat();
        }
