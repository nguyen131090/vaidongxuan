...
use app\modules\whoarewe\models\ItemData;
...
/**
 * Catalog module API
 * @package app\modules\whoarewe\api
 *
...
//])->with(['seo', 'category'])->status(Item::STATUS_ON);

            $query = Item::find()->select([
    'app_whoarewe_items.*', 
    new \yii\db\Expression('IF(on_top IS NULL OR on_top="0", 1, 0) AS on_top_flag')
])->joinWith('category')->with(['seo', 'category'])->andWhere(['app_whoarewe_items.status' => 1]);
            
             if(!empty($options['where'])){
                $query->andFilterHaving($options['where']);
            }
            if(!empty($options['orderBy'])){
//                $query->orderBy(['order_num' => SORT_DESC]);
                $query->addOrderBy($options['orderBy']);
            } 
            else if(!empty($options['orderByCustom'])){
                $query->addOrderBy($options['orderByCustom']);
            } else {
                $query->sortDate();
            }
            ...
            if(count($filters)){
                ...
                foreach($filters as $field => $value){
                    if(!is_array($value)) {
                        ...
                        $filtersApplied++;
                    }  elseif(count($value) == 2 && $value[0] != 'IN'){
                        if(!$value[0]){
                        ...
                        $subQuery->orFilterWhere(['and', ['name2' => $field], $additionalCondition]);

                        $filtersApplied++;
                    } elseif($value[0] == 'IN'){
                        $additionalCondition = ['value' => $value[1]];
                         $subQuery->distinct()->orFilterWhere(['and', ['name' => $field], $additionalCondition]);
                         $filtersApplied++;
                    } 
                }
                if($filtersApplied) {
    ...
    {
        $items = [];
        if(!($item = Item::find()->where(['or', 'slug=:id_slug', 'item_id=:id_slug'], [':id_slug' => $id_slug])->status(Item::STATUS_ON)->orderBy('(slug=:id_slug) DESC, (item_id=:id_slug) DESC', [':id_slug' => $id_slug])->one())){
            return null;
        }
