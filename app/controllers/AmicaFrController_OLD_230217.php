...
//        }
    ...
    public function beforeAction($action) {
        ...
        if(strpos($this->action->id, 'destination') !== false) $this->destination = true;
        if(strpos($this->action->id, 'exclusivites') !== false) $this->exclusives = true;
        if(strpos($this->action->id, 'voyage') !== false) $this->programes = true;
        if(strpos($this->action->id, 'about-us') !== false) $this->aboutUs = true;
   ...
   public function actionExclusivites() {
     ...
     public function actionExclusivitesType() {
        $theEntry = \app\modules\exclusives\api\Catalog::cat(URI);
         ...
         $theEntries = $theEntry->items(['pagination' => ['pageSize' => 8], 'where' => ['status' => 1]]);
...
TXT;
                ...
                // Save db
                $this->saveInquiry($model, 'form_contactce_1216', $data2);
            
            // Email me
            $this->notifyAmica('Contact from ' . $model->email, '//page2016/email_template', ['data2' => $data2]);
            $this->confirmAmica($model->email);
            // Redir
            return Yii::$app->response->redirect(DIR.'merci?from=contactce');
        }

        return $this->render('//page2016/exclusivites-single',[
                    'model' => $model,
   ...
   public function actionFondationThongnongAboutUs() {
    ...
    public function actionResultNosSecretDailleursExclusivites() {
        ...
        $exclusives = \app\modules\exclusives\api\Catalog::items([
            ...
            'filters' => $filters,
            'pagination' => ['pageSize' => Yii::$app->request->get('page') ? 12 : 0]
            ]);
        ...
        if (Yii::$app->request->isAjax) {
            ...
            Yii::$app->session->set('countExcl', count($exclusives));
        }
        $theRaisons = \app\modules\whoarewe\api\Catalog::cat(2);
	...
	public function actionNotreEquipeAboutUs() {
     ...
     public function actionIdeesDeVoyageType() {
        ...
        $this->getSeo($theEntry->model->seo);
         ...
         $theEntries = $theEntry->items(['pagination'=> ['defaultPageSize' => 8]]);
...
TXT;
      if($model->callback == 'Oui'){
            ...
            $this->confirmAmica($model->email);
            // Redir
            return Yii::$app->response->redirect(DIR.'merci?from=booking');
        }
...
TXT;
        ...
        return $this->render('//page2016/idees-de-voyage-entre-ocean-single',[
                    'model' => $model,
                    'allCountries' => $allCountries,
                    'allDialCodes' => $allDialCodes,
                    'root'=> $root,
     ...
     public function actionRechercheIdeesDeVoyage() {
        ...
        if (Yii::$app->request->isAjax) {
            ...
            Yii::$app->session->set('countProg', count($voyage));
        }
      ...
      public function actionNosDestinations() {
        if (Yii::$app->request->isAjax) {
            if(Yii::$app->request->post()['type'] == 'excl'){
                $excl = \app\modules\exclusives\api\Catalog::items();
       ...
       $excl = \app\modules\exclusives\api\Catalog::items();
        Yii::$app->session->set('countVnExcl', count($excl));
    ...
    public function actionNosDestinationsRecherche() {
        ...
        if($length && $length != 'all'){
            if(strpos($length, '-') !== false){
                $filterLen = ['between', 'days', explode('-',$length)[0], explode('-',$length)[1]];
            } else $filterLen = ['>=','days', $length];
        }
        //for country
        $filterCountry = ['countries' => SEG1];
...
TXT;
    ...
    public function actionContact() {
       ...
       // $this->metaD = "Veuillez nous contacter (par email ou directement en remplissant le formulaire) pour toute question ou avis sur votre voyage à l'Indochine";
        ...
        if ($model->load($_POST) && $model->validate()) {
            ...
            if($model->subjet == 'pdv' || $model->subjet == 'dec') {
                ...
                // Save db
                    $this->saveInquiry($model, 'form_contact_1216', $data2);
            ...
            $this->confirmAmica($model->email);
            // Redir
            return Yii::$app->response->redirect(DIR.'merci?from=contact');
        }


        return $this->render('//page2016/contact', [
					'theEntry' => $theEntry,
                    'model' => $model,
                    'allCountries' => $allCountries,
        ]);
    ...
    public function actionRdv() {
       ...
       // $this->metaT = 'Rendez-vous téléphonique';
       // $this->metaD = 'Commencez à concrétiser votre voyage sur mesure au Vietnam, au Laos et au Cambodge par un rendez vous téléphonique avec l’agence locale Amica Travel.';
...
TXT;
                ...
                $this->saveInquiry($model, 'form_rdv_1216', $data2);
            
            // Email me
            $this->notifyAmica('RDV from ' . $model->email, '//page2016/email_template', ['data2' => $data2]);
            $this->confirmAmica($model->email);
            // Redir
            return Yii::$app->response->redirect(DIR.'merci?from=rdv');
        }
            return $this->render('//page2016/rdv', [
						'theEntry' => $theEntry,
                        'model' => $model,
                        'allCountries' => $allCountries,
                        'allDialCodes' => $allDialCodes,
...
TXT;
      if($model->callback == 'Oui'){
            ...
            $this->confirmAmica($model->email);
            // Redir
            return Yii::$app->response->redirect(DIR.'merci?from=devis');
        }

        return $this->render('//page2016/devis', [
					'theEntry' => $theEntry,
                    'model' => $model,
                    'allCountries' => $allCountries,
                    'allDialCodes' => $allDialCodes,
...
TXT;
      if($model->callback == 'Oui'){
            ...
            $this->confirmAmica($model->email);
            // Redir
            return Yii::$app->response->redirect(DIR.'merci?from=votre-projet');
        }
