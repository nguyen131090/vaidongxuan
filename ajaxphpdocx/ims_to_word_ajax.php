...
// $webRoot = 'D:/wamp/www/new-amica.dev';
...
if (!file_exists($webRoot.'/upload/output/devis-ims')) {
    mkdir($webRoot.'/upload/output/devis-ims', 0777, true);
}
...
$docx->replaceTemplateVariableByHTML('devis_number', 'inline', "<span style='font-family:Candara; font-size: 14pt;font-weight:bold;color: #bd3920;'>".$data['devis_number']."</span>");
...
$docx->addTemplateImage('image-2',$webRoot.'/upload/banner_2_devis/'.$data['img_banner_2']);
}
...
$docx->createDocx($webRoot.'/upload/output/devis-ims/' . $data['file_name']);
...
if(strpos($data['template'], '_en') === false && $data['template']!='B2B-FR'){
    ...
    $footer_img = $webRoot.'/helpers/template/devis/footer/banner3/'. $data['template'].'.jpg';

    if($data['footer_image'] != 'false'){
        $footer_img = $webRoot.'/upload/img-devis/footer/'. $data['footer_image'];
    }
    $docx2->replaceHeaderImage('image-3', $footer_img);
    ...
    $merge->mergeDocx($webRoot.'/upload/output/devis-ims/' . $data['file_name'].'.docx', $webRoot.'/helpers/template/devis/footer/'.$nameFt.'.docx', $webRoot.'/upload/output/devis-ims/' . $data['file_name'].'.docx', ['mergeType' => 0]);
}
